...
void init()
{
    ...
    strcpy(&(word[2][0]), "ceil");
    strcpy(&(word[3][0]), "const");
    strcpy(&(word[4][0]), "do");
    strcpy(&(word[5][0]), "end");
    strcpy(&(word[6][0]), "float");
    strcpy(&(word[7][0]), "floor");
    strcpy(&(word[8][0]), "if");
    strcpy(&(word[9][0]), "odd");
    strcpy(&(word[10][0]), "pow2");
    strcpy(&(word[11][0]), "pow3");
    ...
    strcpy(&(mnemonic[ltf][0]), "ltf"); // 新增
    strcpy(&(mnemonic[opr][0]), "opr");
    strcpy(&(mnemonic[lod][0]), "lod");
    strcpy(&(mnemonic[sto][0]), "sto");
    strcpy(&(mnemonic[sta][0]), "sta"); // 新增
    strcpy(&(mnemonic[cal][0]), "cal");
    strcpy(&(mnemonic[inte][0]), "int");
    strcpy(&(mnemonic[jmp][0]), "jmp");
    strcpy(&(mnemonic[jpc][0]), "jpc");
    strcpy(&(mnemonic[lda][0]), "lda"); // 新增
...
int block(int lev, int tx, bool* fsys)
{
    ...
    do {
        ...
        if (sym == flosym) /* 收到变量声明符号，开始处理变量声明 */
        {
            getsymdo;
        ...
        if (sym == readsym) /* 准备按照read语句处理 */
        {
            ...
            if (sym != lparen) {
                ...
                do {
                    getsymdo;
                    format = sym;
                    if (format != flosym && format != varsym) { // 必须显示说明输入格式
                        error(35);
                    } else {
                        getsymdo;
                    }
                    ...
                    if (i == 0) {
                        error(11); /* read()中应是声明过的变量名 */ //和error11一样，从35改成改成11
                    } else {
            ...
            if (sym == writesym) /* 准备按照write语句处理，与read类似 */
            {
                ...
                if (sym == lparen) {
                    ...
                    do {
                        getsymdo;
                        format = sym;
                        if (format != flosym && format != varsym) {
                            error(36);
                        } else {
                            getsymdo;
                        }
                        ...
                        if (format == varsym) { /* 生成输出指令，输出栈顶的值 */
                            gendo(opr, 0, 14);
                        } else {
...
int expression(bool* fsys, int* ptx, int lev)
{
    ...
    if (sym == plus || sym == minus) /* 开头的正负号，此时当前表达式被看作一个正的或负的项 */
    {
        ...
        if (addop == minus) {
            gendo(opr, 0, 1); /* 如果开头为负号生成取负指令，正号就忽略掉 */
        }
    } else /* 此时表达式被看作项的加减 */
    {
        memcpy(nxtlev, fsys, sizeof(bool) * symnum);

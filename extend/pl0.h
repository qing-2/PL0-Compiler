...
#define norw 18 /* 关键字个数，增加float floor ceil pow2 pow3*/
...
instf codef[cxmax]; /* 新增，用于解决浮点型立即数 */
...
#define vardeclarationdo(a, b, c)      \
    ...
    return -1
#define flodeclarationdo(a, b, c)      \
    if (-1 == flodeclaration(a, b, c)) \
    return -1
...
int flodeclaration(int* ptx, int lev, int* pdx); // 新增
